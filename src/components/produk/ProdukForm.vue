...
<template>
    <form @submit.prevent="onSubmit" class="space-y-4 max-w-md mx-auto p-4 bg-white rounded-md shadow">
        ...
        <div>
            ...
            <input id="nama" v-model="form.nama" type="text" required placeholder="Masukkan nama produk" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm
        ...
        <div>
            ...
            <input id="kode_barang" v-model="form.kode_barang" type="text" required placeholder="Masukkan kode barang"
        ...
        <div>
            ...
            <input id="kategori" v-model="form.kategori" type="text" required placeholder="Masukkan kategori" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm
                 focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
        </div>
        ...
        <div>
            ...
            <input id="jumlah" v-model.number="form.jumlah" type="number" min="0" required placeholder="Masukkan jumlah"
                class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm
                 focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
        </div>
        ...
        <div>
            ...
            <input id="harga" v-model.number="form.harga" type="number" min="0" required placeholder="Masukkan harga"
                class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm
                 focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
        </div>
        ...
        <div class="relative">
            <label for="supplier-search" class="block text-sm font-medium text-gray-700 mb-1">Supplier</label>
            <input id="supplier-search" type="text" v-model="supplierSearch" @input="onSupplierSearch"
                @focus="dropdownOpen = true" @keydown.down.prevent="highlightNext()"
                @keydown.up.prevent="highlightPrev()" @keydown.enter.prevent="selectHighlighted()"
                placeholder="Cari supplier..." autocomplete="off" class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm
                 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500" />

            <!-- Clear button -->
            <button v-if="selectedSupplier" type="button" @click="clearSupplier"
                class="absolute right-8 top-2.5 text-gray-400 hover:text-gray-600" aria-label="Clear selection">
                &times;
            </button>

            <!-- Dropdown options -->
            <ul v-if="dropdownOpen && suppliers.length > 0"
                ...
                <li v-for="(supplier, index) in suppliers" :key="supplier.id"
                    @mousedown.prevent="selectSupplier(supplier)" :class="[
...
watch(() => props.produk, async (newVal) => {
    if (newVal) {
        form.value = {
            nama: newVal.nama || '',
