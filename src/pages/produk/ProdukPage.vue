    ...
    <BaseModal :show="showAddModal" title="Tambah Produk Baru" @close="showAddModal = false">
      <div class="grid gap-4 text-sm">
        <div>
          ...
          <input id="nama" type="text" v-model="newItem.nama" class="w-full border px-3 py-2 rounded" />
        </div>

        <div>
          <label for="kode_barang" class="block mb-1 text-gray-600">Kode Barang</label>
          <input id="kode_barang" type="text" v-model="newItem.kode_barang" class="w-full border px-3 py-2 rounded" />
        </div>

        <div>
          <label for="kategori" class="block mb-1 text-gray-600">Kategori</label>
          <input id="kategori" type="text" v-model="newItem.kategori" class="w-full border px-3 py-2 rounded" />
        </div>

        <div>
          <label for="harga" class="block mb-1 text-gray-600">Harga</label>
          <input id="harga" type="number" v-model="newItem.harga" class="w-full border px-3 py-2 rounded" />
        </div>

        <div>
          <label for="jumlah" class="block mb-1 text-gray-600">Jumlah</label>
          <input id="jumlah" type="number" v-model="newItem.jumlah" class="w-full border px-3 py-2 rounded" />
        </div>
        ...
        <div>
          ...
          <div class="relative">
            ...
            />
            <div v-if="filteredSuppliers.length > 0" class="absolute z-10 w-full bg-white shadow-lg mt-1 rounded border">
              <ul>
                <li
