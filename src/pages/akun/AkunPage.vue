<template>
    <div class="px-4 py-6 w-full mx-auto">
        ...
        <div class="flex flex-wrap items-center gap-4 mb-6">
            <input v-model="search" @input="onSearchInput" type="text" placeholder="Cari akun..."
        ...
        <BaseModal :show="showModal" title="Form Supplier" @close="closeModal">
            <SupplierForm :supplier="selectedAkun" @submit="handleSubmit" />
        </BaseModal>
    </div>
</template>
...
import SupplierForm from '../../components/akun/AkunForm.vue';
import {
    getAkunList,
    createAkun,
    resetPassword,
...
const resetPasswordAkun = (akun) => {
    if (window.confirm('Apakah Anda yakin ingin mereset password akun ini?')) {
        loading.value = true;
        resetPassword(akun.id)
            .then(() => {
...
const onSearchInput = () => {
    ...
    searchTimeout = setTimeout(() => {
        page.value = 1; // Reset ke halaman pertama saat pencarian baru
        fetchData();
